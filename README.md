# yokemate

Оркестраторский чат для разработки по многим организациям и репозиториям: обычная сессия Claude Code в корне этого репозитория, открытая в `herdr`, плюс семь скиллов и полтора десятка команд вокруг неё. Тикеты — из YouTrack, состояние — в SQLite, режимы работают в собственных панелях.

## Быстрый старт

```bash
pnpm on-me                                        # тикеты на мне, по всем YouTrack
pnpm add-project <клон> --tracker acme:ACME --model opus   # подключить репозиторий
```

Дальше в главном чате: `/plan ACME-347` → `/do` → `/review` → `pnpm accept`.

## Документы

- [`CLAUDE.md`](./CLAUDE.md) — правила и команды оркестратора; их читают агенты.
- [`docs/usage.md`](./docs/usage.md) — инструкция инженера: от подключения проекта до приёмки.
- `home/knowledge/<org>/<project>/` — глоссарии, ADR и артефакты задач; `home/journal/` — журнал пула.
