...
Явный `/plan KEY1 KEY2` создаёт отдельный tab на каждый ключ; `/plan --split KEY1 KEY2` и `/split plan KEY1 KEY2` — отдельный split на ключ от той же вызывающей pane. Каждый worker получает только свой ключ, свою модель паспорта и отправляет отдельный результат. Общие `--model` и буквальный хвост передаются каждому; отказ одного ключа не останавливает остальные. Single-key и ввод проблемы/смешанного текста остаются одной разговорной surface, включая пакет тикетов после интервью.
