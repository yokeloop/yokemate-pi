...
Never drive the engineer's browser — no claude-in-chrome, no CDP into their instance; when a look is needed, ask the engineer for a screenshot. Kill only processes you started, by saved PID — never pkill — and stop them all when the review ends. What the engineer corrects about the stand goes into `knowledge/<org>/<repo>/flow.md` — the next review must not relearn it.
