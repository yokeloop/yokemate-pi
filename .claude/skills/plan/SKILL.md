...
Every question goes through `AskUserQuestion` — one call, one question, then wait. A header of ≤12 characters naming the decision; 2–4 options, the recommended one first and labelled `(рекомендую)`, one line of reasoning in each option's description. The remaining options are the strongest real alternatives, not strawmen. «Other» is always there — the options are a starting point, not a cage. Facts you established go *before* the call, in two or three lines, and the question itself stays one question — a wall of findings with a question at the end is not an interview.
...
Then one `AskUserQuestion`: «Заводим?» — «Заводим» / «Есть замечания». On remarks: fix what they name and print the corrected package again, still creating nothing. Only «Заводим» opens what follows.
...
The `run` verdict changes only the ending. When every plan of the run is recorded, send the outcome to the main chat as a courtesy. The report is a few lines: the keys and their plan paths, ready for `/do`. Deliver it by these steps:
...
4. Zero or several candidates, or the retry from step 3 failed too → say the report in this pane — the plans are already recorded either way.
