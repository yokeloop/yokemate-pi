...
- A mode starts only within the engineer's current request. Every `/plan` shows a concise approach after complete reconnaissance and requires the next real interactive reply before writers or record, even when there are no forks; that approval is plan-only. Plain `/plan KEY` always stops at a recorded plan. Automatic initial do requires a parent-owned explicit plan+do receipt from the same interactive input and `workflowApproval=false`; true always stops after planning. Subsequent explicit `/do` or unambiguous model-mediated approval is the approval, not a reason for another confirm. The one review exception is an unambiguous final rework verdict in its registered live review runtime: the parent records the exact rework plan and starts one do without another confirm under either `workflowApproval` value. Settings never create authority. Only exact interactive `/ship` permits merge; its second UI confirm is optional.
...
- `home/pool.json` — partial map последнего model fallback по модам `plan, review, do, ship, worklog, note, research`; `pool.do` также даёт concrete default новому паспорту без scalar `--model`; правится руками, едет с `home/`
...
- `pnpm add-project <clone-path> --tracker <name:KEY> [--model <m>] [--model <mode>=<m> …] [--figma <mcp>] [--figma-file <url>] [--subsystem <value>]` — connect a repository (imports its `.yoke/` knowledge once). Scalar `--model` is the project's concrete default; without it, `pool.do` is validated and written as that same concrete value to SQLite and `home/projects.json`. Repeated `<mode>=<m>` values are mode overrides and never substitute for the scalar default. `--subsystem` only groups `on-me` output where several repositories share one tracker project; the value is checked against the tracker's enum. `--tracker github:PREFIX` connects a GitHub-Issues repository; `--subsystem` is YouTrack-only.
...
- `pnpm plan <TICKET> <absolute-plan-path> --content-hash <reviewed-sha256>` — record the exact reviewed plan bytes, stage → planned; the /plan flow runs it itself
- `pnpm spawn <TICKET> [<TICKET> …] [--plan <path>] [--model <m>]` — route one parent-owned do list through the live runtime; every admissible key is reserved before bounded per-key coordinators start, and stage → running only after that key's preparation. The plan comes from each ticket's row; `--plan` must match the current recorded binding.
- `pnpm review <TICKET> [note]` · `pnpm ship <KEY> [<KEY> …] [note]` · `pnpm worklog <org> [note]` — ship routes one parent-owned list to independent per-key background coordinators; review and worklog open a new tab by default, or a split with `--split`. The note reaches every ship worker verbatim. When the engineer names a model in their command («запусти на gpt-6-astra»), translate it into `--model <m>` — spawn takes the flag too. Priority is explicit model → matching passport mode override → passport default → `pool[mode]` only when the passport query has zero rows. Passport disagreement or `NULL` refuses without reading the pool.
- `coordinator_merge` — the ship coordinator's only merge path: the live parent verifies the owned run and exact prepared PR, serializes contenders by canonical repository/base, rereads the PR target/head, runs the per-part fresh gate and passes that exact SHA to GitHub's match-head merge
- `pnpm research [--project <org/repo|repo|KEY>] [--model <m>] [--topic] <text…>` — ticketless research in a new tab by default (`--split` for a split); it reads a selected passport or free topic without queue/stage/worktree effects, and it stays open until the engineer closes it. A selected project is the exception to general pool fallback: explicit model → research override → project default → refusal; only a free topic uses `pool.research`. Every clone edit needs one fresh TUI consent for its exact target and final diff; `-a`/YOLO does not bypass it.
...
- `pnpm split plan [--split] [KEY …|проблема] [--model <m>] [note]` — the /plan launcher, a new tab by default or a split of the calling pane with `--split`; the panes are conversational — the engineer closes them. По проблеме, без ключа, модель приходит из `home/pool.json`
...
- `pnpm accept <TICKET> [--rework <plan-path>]` — acceptance outcome, run by the review pane at the engineer's verdict; `--rework` names the rework plan's path outright. In a registered review runtime the parent verifies that final verdict, records the exact plan and waits for one do work ACK before closing that exact surface; failure stays open and is never retried from the old verdict. An unstamped main call remains repair-record-only. A clean pass removes the queue row itself; the task folder stays — /ship works in it and removes it after the merge
...
The five interactive commands `/plan`, `/review`, `/worklog`, `/note` and `/research` open a new herdr tab by default. Use `/<mode> … --split` before the first `--` for a split of the calling pane; `/plan --split YM-1` is the primary form, `/split plan YM-1` its compatibility alias only. There is no `--tab`. After `--`, every token is literal worker text, including `--split` and `--model`. Successfully launched surfaces remain conversational until the engineer closes them.

Typed launchers in `.pi/prompts/` run `where → launch` and stop in the main chat. Worker skills do the work in the stamped mode surface. Explicit `/plan KEY1 KEY2`, `/do KEY1 KEY2` and `/ship KEY1 KEY2` are parent-owned list runs: all admissible keys are reserved first, bounded per-key workers start independently, refusals do not stop siblings, per-key terminal results may arrive in completion order, and one aggregate preserves input order. `/plan` opens one tab per explicit key (`--split` opens one split per key); each worker has only its own identity and passport model. Single-key and problem/mixed plan input stays one surface; a problem interview's newly created package stays together. `/journal` and `/warmup` remain inline skills.

`/do` and `/ship` launch background coordinators, with one exception: `/do <ROOT>` for an activated native tracker subtree opens one conversational root group-do tab over bounded background member coordinators. The group has one immutable revision, durable queued/running/ready/integrated/blocked members, internal member PRs into `<ROOT>`, root own-work from `<ROOT>-own`, and one final PR per repository. `integrated` means verified inclusion plus the intermediate tracker effect, not Done. `/ship` remains background-only and releases only accepted final PRs; partial merge or tracker completion is reported without claiming done. One exact `/ship` permit authorizes the ordered list once; each key then runs independently. Merging requires the engineer's explicit `/ship`, and shared repository/base merge sections serialize through the parent-owned `coordinator_merge`; no CLI or direct API path presses that button.
