...
- A mode starts only within the engineer's current request. Plain `/plan KEY` always stops at a recorded plan. Automatic initial do requires a parent-owned explicit plan+do receipt from the same interactive input and `workflowApproval=false`; true always stops after planning. Subsequent explicit `/do` or unambiguous model-mediated approval is the approval, not a reason for another confirm. Settings never create authority. Only exact interactive `/ship` permits merge; its second UI confirm is optional.
