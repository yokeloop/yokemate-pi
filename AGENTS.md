...
- `pnpm spawn <TICKET> [<TICKET> …] [--plan <path>] [--model <m>]` — route a do background coordinator through the live parent runtime; stage → running only after preparation and reservation. The plan comes from the ticket's row; `--plan` must match its current recorded binding
...
Typed launchers in `.pi/prompts/` run `where → launch` and stop in the main chat. Worker skills do the work in the stamped mode surface. `/plan <KEY> [<KEY> …]` takes existing ticket keys in input order, or `/plan <problem>` takes a problem statement. It uses the same `.pi/skills/plan/SKILL.md` in the new surface, with scout/writer subagents and one `pnpm plan` record per key. An explicit `/plan KEY1 KEY2` opens one independent tab per key in input order; `--split` (also `/split plan`) opens one split per key from the same parent pane. Each worker has its own `YOKEMATE_TICKET=<KEY>`, prompt `/skill:plan <KEY>`, passport model and separate parent report. Explicit `--model` and literal tail apply to each worker; a refused/failed key does not stop siblings, and the launcher returns all result lines with a nonzero exit on any failure. Single-key and problem/mixed input keep one surface; a problem interview’s newly created package stays together. `/journal` and `/warmup` remain inline skills.
