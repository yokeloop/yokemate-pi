...
The task tab's subagents live in `.pi/agents/do/`; `spawn` links them into `work/<TICKET>/.pi/agents/` at launch and they die with the task folder — nothing is placed outside yokemate for this. They are raised by the `subagent` tool of the `.pi/extensions/subagent/` extension, which runs a child `pi` per call; a subagent's final message never returns as the tool result: every call is detached, the tool returns at once, and the report arrives in the chat as a `subagent-report` message — so the chat keeps answering while agents run, and `/do` collects its executors' reports the same way. Subagents themselves send no messages.
