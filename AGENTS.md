...
- `pnpm ship-merge <KEY> <PR-URL> <--merge|--squash|--rebase>` — the only ship merge path: serialize contenders by remote/base, reread the PR target/head under the lock, run the fresh gate and pass that exact SHA to GitHub's match-head merge
...
`/do` and `/ship` launch only background coordinators, with no tab or split. One exact `/ship` permit authorizes the ordered list once; each key then runs independently. Merging requires the engineer's explicit `/ship`, and shared remote/base merge sections serialize through `pnpm ship-merge`; no other path presses that button.
