...
`/plan <KEY> [<KEY> …]` takes existing ticket keys in input order, or `/plan <problem>` takes a problem statement, and records each plan with `pnpm plan`; reconnaissance and plan-writing run in the `plan-scout` and `plan-writer` subagents (root `.pi/agents/`, in git). One skill, no worker pair — its `launch` verdict means «work inline right here»; the mechanics live in `.pi/skills/plan/SKILL.md`.
