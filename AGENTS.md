...
- `pnpm test` — reduced suite without real Pi/herdr or nested test runners, one file worker; Node/socket/git/package fixtures remain. Real-runtime integration coverage is intentionally deferred; runtime is unmeasured until an explicitly authorized check. `pnpm metrics` — dialog-quality numbers on demand
...
`/do` and `/ship` launch only background coordinators, with no tab or split. One exact `/ship` permit authorizes the ordered list once; each key then runs independently. Merging requires the engineer's explicit `/ship`, and shared repository/base merge sections serialize through the parent-owned `coordinator_merge`; no CLI or direct API path presses that button.
