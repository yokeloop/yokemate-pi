...
for (const ownWorkspace of [false, true]) test(`real pnpm installs inside nested worktree (own workspace: ${ownWorkspace})`, () => {
  ...
  try {
    ...
    const recipe = recipeFor(s.worktree);
    assert.ok("command" in recipe);
    const scope = { root: s.root, dataRoot: join(s.root, "home"), cwd: s.worktree, ticket: s.ticket, project: JSON.stringify([s.repo]) };
    assert.equal(judge("do", "Bash", { command: recipe.command.join(" ") }, scope), null);
    assert.equal(judge("do", "Bash", { command: "pnpm install --frozen-lockfile" }, scope)?.decision ?? null, ownWorkspace ? null : "deny");
