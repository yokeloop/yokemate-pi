...
const baseScenario = scenario.replace(/-tool$/, "");
...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerCommand("wf-new", { handler: async (_args, ctx) => { await report("lifecycle_new"); await ctx.newSession(); } });
  pi.registerCommand("wf-fork", { handler: async (_args, ctx) => { await report("lifecycle_fork"); const entry = ctx.sessionManager.getBranch().find((item: any) => item.type === "message" && item.message?.role === "user"); if (entry) await ctx.fork(entry.id, { position: "at" }); } });
  pi.registerCommand("wf-reload", { handler: async (_args, ctx) => { await report("lifecycle_reload"); await ctx.reload(); } });
  pi.on("session_start", async (_event, ctx) => {
    if (process.env.WORKFLOW_INPUT_CHILD) process.argv[1] = process.env.WORKFLOW_INPUT_CHILD;
  ...
  pi.on("session_shutdown", (event) => { void report("session_shutdown", { reason: event.reason }); });
  ...
  pi.on("tool_execution_end", (event) => { if (event.toolName === "subagent") void report("tool_end", { isError: event.isError, resultCode: hash(JSON.stringify(event.result)) }); });
  pi.registerProvider("workflow-input-fixture", {
    ...
    streamSimple(model, context, options) {
      ...
      void (async () => {
        ...
        if (!extraction) {
          mainCalls += 1;
          await report("main_enter", { contextHash: hash(JSON.stringify(context.messages)), tools: context.tools?.map((tool) => tool.name) ?? [] }, baseScenario === "hold-main");
          ...
          if (scenario.endsWith("-tool") && mainCalls === 1) {
            if (baseScenario === "near-timeout") await new Promise<void>((resolve) => setTimeout(resolve, 14000));
        ...
        options?.signal?.addEventListener("abort", () => { void report("extraction_abort"); }, { once: true });
        ...
        await report("extraction_released");
        if (baseScenario === "error") throw new Error("SENTINEL_PRIVATE_PROVIDER_ERROR");
        if (baseScenario === "timeout" || baseScenario === "near-timeout" || baseScenario === "late-success") {
          await new Promise<void>((resolve) => setTimeout(resolve, 16000));
          await report(options?.signal?.aborted ? "extraction_late_after_abort" : "extraction_late_without_abort");
        }
        let value: unknown = { kind: "none" };
        if (baseScenario === "approval") value = { kind: "advance-plan-do", ticket: "YM-1", binding: null, actions: ["plan", "do"], evidence: [{ start: 0, end: raw.length, text: raw }] };
        if (baseScenario === "ready") value = { kind: "approve-ready-do", ticket: "YM-1", binding: bindings[0]?.contentHash ?? null, actions: ["do"], evidence: [{ start: 0, end: raw.length, text: raw }] };
        if (baseScenario === "malformed") value = { kind: "advance-plan-do", ticket: "YM-1" };
