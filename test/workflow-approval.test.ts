...
test("explicit execution after save-only record still requires model and exact ready binding", () => {
  ...
  for (const raw of ["Запускай YM-1", "Run YM-1", "Выполни YM-1"]) {
    assert.equal(isWorkflowCandidate(raw), true, raw);
    assert.deepEqual(validateExtraction(make(raw), raw, [binding]), make(raw));
    assert.throws(() => validateExtraction(make(raw), raw, []), /binding/);
    assert.throws(() => validateExtraction(make(raw), raw, [{ ...binding, contentHash: "changed" }]), /binding/);
  }
  for (const raw of ["«Запускай YM-1»", "Run YM-1?", "If ready, run YM-1", "Не запускай YM-1", "Explain run YM-1", "Запускай YM-1 и YM-2"]) assert.throws(() => validateExtraction(make(raw), raw, [binding]), /extraction/);
});
