...
test("only a live blocked plan lineage can continue one registered candidate generation", async () => {
  ...
  writeFileSync(join(runtimeDir, "main-pane.json"), JSON.stringify({ pid: process.pid, starttime, sessionId: target.sessionId, parentPane: null, cwd: root, mode: "main", ticket: null }));
  writeFileSync(join(runtimeDir, "plan-pane.json"), JSON.stringify({ pid: process.pid, starttime, sessionId: "plan-session", parentPane: "main-pane", cwd: root, mode: "plan", ticket: "YM-1" }));
