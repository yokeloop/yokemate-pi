...
test("raw interactive authority flows through real plan CLI and parent control without a second do confirm", { timeout: 300000 }, async () => {
  ...
  try {
    ...
    const runOwnedPublicationCase = async (label: string, scoutExpected: ExpectedPublication, planExpected: ExpectedPublication, authority: "none" | "plain" | "advance" | "guarded" = "none", surface: "tab" | "split" = "tab") => {
      ...
      const expectedPlan = readFileSync(plan);
      const workerPromise = promisify(execFile)(process.execPath, ["--experimental-strip-types", "--no-warnings", join(source, "test", "fixtures", "workflow-plan-worker.mjs"), dir, plan, source, ownedRunId, ownedPane, `owned-${label}-session`], { cwd: dir, env: { ...process.env, WORKFLOW_PLAN_CONTENT_BASE64: expectedPlan.toString("base64"), WORKFLOW_PLAN_WORKER_SCOUT_RESULT: scoutResultFile, WORKFLOW_PLAN_WORKER_CONTINUE: continueFile, ...(label === "plan-pending" ? { WORKFLOW_FOREIGN_SCOUT: "1" } : {}) }, maxBuffer: 4 * 1024 * 1024 });
      ...
      rmSync(plan);
      assert.equal(existsSync(plan), false, `${label}: writer must create the plan artifact`);
      ...
      try {
        ...
        const workerResult = JSON.parse(workerMatch[1]!) as { writer: any; foreign?: { state: string; reason?: string }; proof: { writeToolEvents: number; bytes: number; hash: string; mtimeNs: string }; stdout: string; stderr: string };
        ...
        assert.equal(workerResult.proof.writeToolEvents, 1, label);
        assert.equal(workerResult.proof.bytes, expectedPlan.length, label);
        assert.equal(workerResult.proof.hash, createHash("sha256").update(expectedPlan).digest("hex"), label);
    ...
    const saveOnlyExpectedPlan = readFileSync(plan);
    rmSync(plan);
    assert.equal(existsSync(plan), false, "save-only writer must create the plan artifact");
    const noIdWorker = await promisify(execFile)(process.execPath, ["--experimental-strip-types", "--no-warnings", join(source, "test", "fixtures", "workflow-plan-worker.mjs"), dir, plan, source, "-", "save-only-pane", "save-only-session"], { cwd: dir, env: { ...process.env, PI_SESSION_ID: "save-only-session", WORKFLOW_PLAN_CONTENT_BASE64: saveOnlyExpectedPlan.toString("base64") }, maxBuffer: 4 * 1024 * 1024 });
    const noIdMatch = noIdWorker.stdout.match(/PLAN_WORKER_RESULT (.+)$/m);
    assert.ok(noIdMatch, noIdWorker.stdout + noIdWorker.stderr);
    const noIdResult = JSON.parse(noIdMatch[1]!) as { scout: any; writer: any; proof: { writeToolEvents: number; bytes: number; hash: string; mtimeNs: string }; stdout: string; stderr: string };
    assert.equal(noIdResult.writer.planResult?.state, "verified");
    assert.equal(noIdResult.writer.planResult?.source, "reconciled");
    assert.equal(noIdResult.proof.writeToolEvents, 1);
    assert.equal(noIdResult.proof.bytes, saveOnlyExpectedPlan.length);
    assert.equal(noIdResult.proof.hash, createHash("sha256").update(saveOnlyExpectedPlan).digest("hex"));
