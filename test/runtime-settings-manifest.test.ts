...
test("public regression manifest consumes exact passed case IDs for every runtime setting cell", { timeout: 180000 }, () => {
  ...
  for (const file of [...new Set(adapters.map((adapter) => adapter.file))]) {
    ...
    const result = spawnSync(process.execPath, ["--experimental-strip-types", "--no-warnings", "--test", `--test-name-pattern=${pattern}`, new URL(file, import.meta.url).pathname], { encoding: "utf8", timeout: 150000, env });
