...
export class CoordinatorRegistry {
  ...
  reserve(request: CoordinatorRequest, origin: CoordinatorOrigin, parentSessionId: string, model: string, cwd: string, projects: string[], rejectDuplicate: boolean, reserved?: { runId: string; parentRunId: string }): CoordinatorRun {
    ...
    const runId = reserved?.runId ?? randomUUID();
    if (this.runs.has(runId)) throw new Error(`coordinator run ${runId} is already reserved`);
    const identity: RuntimeIdentity = { runId, parentRunId: reserved?.parentRunId, listRunId: reserved?.parentRunId, keyRunId: reserved?.runId, parentSessionId, mode: request.mode, ticket: request.tickets.join("+"), project: projects, role: "coordinator", cwd, model };
