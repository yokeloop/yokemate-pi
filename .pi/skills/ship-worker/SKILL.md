...
6. **Serialized fresh gate → merge; refusal → stop.** From the yokemate-pi instance root run `pnpm ship-merge <KEY> <url> --<method>` for each PR, using the repository method established in step 1. The command locks that remote/base against sibling ship coordinators, rereads the current PR target and head inside the lock, runs the gate there, and passes exactly that fresh head to GitHub's match-head merge. Never call `gh pr merge` directly. A `ship-merge: …` refusal, or red you cannot make green inside the ticket's scope → do not merge: put the literal output in the report and leave the folder and remaining PRs as they stand.
