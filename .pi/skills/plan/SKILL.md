...
3. **Plan** — spawn the `plan-writer` subagent: the key, the project, the scout's facts and every decision made above. It writes the plan in the shape `PLAN-FORMAT.md` defines to `home/knowledge/<org>/<project>/ai/<KEY>-<slug>/<KEY>-<slug>-plan.md` and returns the path. The tool returns at once: the path arrives later in a message opening with `[subagent plan-writer]` — the chat stays answerable meanwhile, and on that message you continue with «Record» below. A report opening with `[subagent plan-writer failed]` → call `plan_finish` with `outcome: "blocked"` and the literal reason, say it in one line in the feed and stop. Read the result: an open question left in it is a defect — send it back with the decisions it missed.
4. **Record** — `pnpm plan <KEY> <plan-path>`, then retain the key and plan path for the single outcome report after this worker’s key (or every ticket created by its problem interview) has been processed. Report the actual parent handoff output. Without advance authority, or when `workflowApproval=true`, say it is ready for `/do` and stop. With an existing advance receipt and false, `pnpm plan` hands the recorded binding to the live parent, which launches exactly once; report its actual run ID or literal refusal. Never call `pnpm spawn` to imitate this handoff, never supply an authority flag, and never ask a second generic permission to repair a refused receipt. Report this worker’s outcome as described below.
...
One report in the feed after the run, for this worker’s key (or one line per ticket created by its problem interview in order): key — plan path, or key — refusal reason. Every successful ticket has its plan recorded and `pnpm queue` showing `planned`. A ready plan is not do approval. An explicit subsequent `/do` or unambiguous natural-language approval creates a fresh parent-owned receipt on the current recorded plan; changed content or scope requires new approval. When there is no live parent, the plan stays recorded and no automatic do starts.
