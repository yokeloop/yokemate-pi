...
3. **Plan** — spawn the `plan-writer` subagent with the key, project, every decision and the original ticket, and pass `acceptedInputId` from the accepted scout artifact (or the accepted-input ID shown by audited recovery). The runtime injects the immutable full scout bytes; never paste or substitute an envelope preview or `{previous}` as the source. It writes the plan in the shape `PLAN-FORMAT.md` defines to `home/knowledge/<org>/<project>/ai/<KEY>-<slug>/<KEY>-<slug>-plan.md` and returns only the path. The tool returns at once. Accept only its correlated valid terminal envelope. Read the saved plan yourself, verify its ordered form and decisions, and fix any open question through the writer before Record; the writer never publishes its draft.
4. **Record** — `pnpm plan <KEY> <plan-path>`. Both owned and legacy entries preflight the current accepted scout's complete local artifact, validate the plan format and safety, snapshot the exact reviewed bytes, and repeat the same binding checks under the record lock before the `planned` CAS. Only after a successful local record do they attempt the separate scout and plan comments as best-effort publications. Retain the key, path and each publication outcome for the report. Exit zero and «ready» follow the verified local record even when either comment is pending; warn with document kind, safe target and code, and never call a pending document published. A local artifact or binding failure blocks record/ready, and a binding change after CAS is reported literally as locally recorded with `binding_changed` and no handoff. Without advance authority, or when `workflowApproval=true`, the locally recorded plan is ready for `/do`. With an existing advance receipt and false, report the actual run ID or literal refusal. Never call `pnpm spawn`, add authority flags, or duplicate comments manually.
