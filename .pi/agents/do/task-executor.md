...
- The parent prepared the worktree before you started. Use the project's documented environment; for Node, tools come from this worktree's `node_modules/.bin` (`pnpm exec …`). A missing tool starts diagnosis: inspect resolution, workspace root and versions, and make safe project-local setup repairs using documented bootstrap/frozen install commands. Do not change manifests/lockfiles solely to make setup pass, borrow another tree's dependencies, or use `npx -y`/a global compiler instead of a required local tool. Report the repair and checks to the parent so it can renew the readiness receipt; an unresolved environment problem names the cause and attempted repairs, not just the first install failure.
...
- Done means: every step's check green, the project's own build passes locally, checks ran through its documented environment (Node tools from the worktree's own `node_modules/.bin`), ready for review.
