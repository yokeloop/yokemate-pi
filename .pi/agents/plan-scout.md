...
- **Is it solvable with what is given?** Establish the facts from primary sources: the affected repositories and their state, the code paths involved, configs, versions, related PRs and commits. Read, run read-only, check — do not ask what you can look up. Name the applicable glossary and ADRs with exact paths and sections, the definitions they establish, any conflicts with the ticket or code, and the absence of documentation when none applies. Separate present facts, past decisions, and proposals. Not solvable → stop here and report exactly what is missing. No plan gets written around a hole.
